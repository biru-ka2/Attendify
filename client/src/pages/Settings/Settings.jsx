...
const Settings = () => {
  ...
  useEffect(() => {
    if (student) {
      ...
      const s =
        student.subjects && student.subjects instanceof Map
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      setSubjects(
        res.data.student.subjects instanceof Object
          ? res.data.student.subjects
          : Object.fromEntries(res.data.student.subjects || [])
      );
      setNewSubjectName("");
      setNewSubjectCode("");
  ...
  const confirmDeleteProfileImage = async () => {
    ...
    try {
      const res = await axiosInstance.delete(
        API_PATHS.STUDENT.DELETE_PROFILE_IMAGE
      );
      if (res.data.success) {
  ...
  const saveProfile = async () => {
    ...
    try {
      ...
      const res = await axiosInstance.put(
        API_PATHS.STUDENT.UPDATE_PROFILE || "/api/student/profile",
        payload
      );
          ...
          >
            <div
              style={{
                width: 84,
                height: 84,
                borderRadius: "50%", // better than 9999
                overflow: "hidden",
                background: "#f3f4f6",
                display: "flex",
                alignItems: "center",
              ...
              {profileImagePreview ? (
                <img
                  ...
                  style={{
                    ...
                    height: "100%",
                    objectFit: "cover",
                    borderRadius: "50%", // <-- force circular image
                  }}
                />
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              ...
              <div className="profile-image-cta">
                <button
                  ...
                  disabled={isUpdatingImage || !profileImageFile}
                >
                ...
                <button
                  ...
                  onClick={requestDeleteProfileImage}
                  disabled={isUpdatingImage || !student?.profileImageUrl}
                >
                  Delete Image
                </button>
              </div>
              {profileImageFile && (
                <div className="muted">Selected: {profileImageFile.name}</div>
              )}
            </div>
          </div>
        </div>
  ...
  const renderSubjectsSection = () => (
    <div>
      ...
      <p className="muted">
        Add or remove subjects for your profile. Changes sync to your account.
      </p>

      <div className="subject-list">
        {Object.keys(subjects || {}).length === 0 && (
          <div className="muted">No subjects found.</div>
        )}
        {Object.entries(subjects || {}).map(([name, code]) => (
          <div className="subject-row" key={name}>
            <div className="subject-info">
              <strong>
                {name} <span className="font-light">- ({code})</span>
              </strong>
            </div>
            <div>
              <button
        ...
        <div className="settings-panel">
          <div className="card-header">
            <div className="tabs">
              ...
              <button
                className={`tab-pill ${
                  selectedSection === "subjects" ? "active" : ""
                }`}
