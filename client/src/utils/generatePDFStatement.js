...
export const generateAttendancePDF = async (studentData, fromDate, toDate, onSuccess, onError) => {
  ...
  const totalPresent = studentData.overall.present;
  const totalClasses = studentData.overall.totalClasses;
  const overallPercentage = studentData.overall.percentage;
  ...
  doc.setFont('helvetica', 'normal');
  doc.setTextColor(0, 0, 0);
  doc.text(`Full Name: ${studentData.name}`, 16, 56);
  doc.text(`Roll Number: ${studentData.rollNo}`, 16, 62);
  ...
  autoTable(doc, {
    ...
    body: [[
      ...
      totalPresent, 
      `${overallPercentage}%`,
      overallPercentage >= 75 ? 'SATISFACTORY' : 'CRITICAL'
    ]],
    theme: 'grid',
  ...
  doc.setFont('helvetica', 'bold');
  doc.setTextColor(70, 130, 180);
  doc.text('SUBJECT-WISE ATTENDANCE DETAILS', 14, doc.lastAutoTable.finalY + 15);

  const subjectTableBody = studentData.subjects.map((subject, index) => {
    const sub = filteredAttendance[subject];
    const absent = sub.totalClasses - sub.present;
    const percent = sub.totalClasses === 0 ? '0.0' : ((sub.present / sub.totalClasses) * 100).toFixed(1);
  ...
  studentData.subjects.forEach((subject, subjectIndex) => {
    const sub = filteredAttendance[subject];
    ...
    doc.setFont('helvetica', 'bold');
    doc.setTextColor(70, 130, 180);
    doc.text(`DAILY ATTENDANCE RECORD - ${subject.toUpperCase()}`, 14, doc.lastAutoTable.finalY + 15);
    
    const detailRows = sub.presentDates.map((date, idx) => {
  ...
  // 🔹 Verification Seal
  const sealY = studentData.isCritical ? doc.lastAutoTable.finalY + 60 : doc.lastAutoTable.finalY + 20;
  ...
  doc.text(`Student Roll No: ${studentData.rollNo}`, 45, footerY + 14);
  ...
  const fileName = `${studentData.name.replace(/\s+/g, '_')}_Attendance_Statement_${timestamp}.pdf`;
